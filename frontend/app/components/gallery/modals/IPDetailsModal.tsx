...
const LicenseOfferCard = ({ offer, onBuy, isBuying }: { offer: LicenseOffer, onBuy: () => void, isBuying: boolean }) => {
 ...
 function IPDetailsModal({ isOpen, onClose, ip }: ModalProps) {
    ...
    const ipExplorerUrl = `https://testnet.seistream.app/tokens/0x043fdf7f85030b6cf49124675987a8526d495fd8/${ip.tokenId}?standart=erc721`
                        ...
                        <div className="p-6 overflow-y-auto">
                            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                                ...
                                <div className="lg:col-span-2 space-y-8">
                                    ...
                                    {/* AI Summary for all types, with custom rendering for dataset */}
                                    {(ip.metadata?.optional?.ai_summary || ip.metadata?.ai_summary) && (
                                        <div>
                                            ...
                                            {/* Custom rendering for dataset ai_summary */}
                                            {ip.metadata?.ip_type === 'dataset' && typeof ip.metadata?.optional?.ai_summary === 'object' ? (
                                                <div className="space-y-2 bg-gray-800/50 p-4 rounded-lg">
                                                    <div>
                                                        <span className="font-semibold text-cyan-300">Use Cases:</span>
                                                        <ul className="list-disc ml-6 text-gray-300">
                                                            {ip.metadata.optional.ai_summary.use_cases?.map((uc: string, idx: number) => (
                                                                <li key={idx}>{uc}</li>
                                                            ))}
                                                        </ul>
                                                    </div>
                                                    <div>
                                                        <span className="font-semibold text-cyan-300">Field:</span> <span className="text-gray-300">{ip.metadata.optional.ai_summary.field}</span>
                                                    </div>
                                                    {ip.metadata.optional.ai_summary.synthesis && (
                                                        <div>
                                                            <span className="font-semibold text-cyan-300">Synthesis:</span>
                                                            <ul className="list-disc ml-6 text-gray-300">
                                                                <li>Location: {ip.metadata.optional.ai_summary.synthesis.location}</li>
                                                                <li>Method: {ip.metadata.optional.ai_summary.synthesis.method}</li>
                                                            </ul>
                                                        </div>
                                                    )}
                                                    <div>
                                                        <span className="font-semibold text-cyan-300">Summary:</span>
                                                        <p className="text-gray-300">{ip.metadata.optional.ai_summary.concise_summary}</p>
                                                    </div>
                                                </div>
                                            ) : (
                                                <p className="text-sm text-gray-400 whitespace-pre-wrap bg-gray-800/50 p-4 rounded-lg">
                                                    {typeof (ip.metadata?.optional?.ai_summary || ip.metadata?.ai_summary) === 'object'
                                                        ? JSON.stringify(ip.metadata?.optional?.ai_summary || ip.metadata?.ai_summary, null, 2)
                                                        : (ip.metadata?.optional?.ai_summary || ip.metadata?.ai_summary)}
                                                </p>
                                            )}
                                        </div>
