...
contract IPNFT is ERC721, ERC721URIStorage, ERC2981, Ownable {
    struct IPInfo {
        ...
        uint256[] derivativeTokenIds;
    }

    mapping(uint256 => IPInfo) public ipInfo;
    uint256 private _nextTokenId = 1;
    address public governance; // governance (Desci) contract that can suspend IPs
    ...
    // }
        ...
        ipInfo[tokenId] = IPInfo({
            ...
            contentHash: contentHash,
            paymentSplitter: address(splitter),
            derivativeTokenIds: new uint256[](0)
        });
    ...
    function addDerivative(uint256 parentTokenId, uint256 derivativeTokenId) external onlyOwner {
        require(_exists(parentTokenId), "parent token doesn't exist");
        require(_exists(derivativeTokenId), "derivative token doesn't exist");
        
        ipInfo[parentTokenId].derivativeTokenIds.push(derivativeTokenId);
    }

    function getDerivatives(uint256 tokenId) external view returns (uint256[] memory) {
        require(_exists(tokenId), "no token");
        return ipInfo[tokenId].derivativeTokenIds;
    }
    ...
    /**
     * Governance wiring and suspension controls
     */
    event GovernanceSet(address indexed governance);
    event IPSuspensionSet(uint256 indexed tokenId, bool suspended);
    ...
    function setSuspended(uint256 tokenId, bool suspended_) external {
        require(msg.sender == governance, "not governance");
        require(_exists(tokenId), "no token");
        _suspended[tokenId] = suspended_;
        emit IPSuspensionSet(tokenId, suspended_);
    }
    ...
    // Block transfers of suspended IPs
    function _beforeTokenTransfer(
        ...
        if (from != address(0) && to != address(0)) { // not mint or burn
            require(!_suspended[tokenId], "ip suspended");
        }
    }
}